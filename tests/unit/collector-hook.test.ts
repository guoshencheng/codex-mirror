...
describe('Codex hook normalization', () => {
  it('maps supported lifecycle hooks and discards prompt, tool input, output, and paths', () => {
    const hook = normalizeHook({
      hook_event_name: 'PreToolUse', session_id: 'codex-session', turn_id: 'turn-1', cwd: '/Users/alice/private/project',
    ...
    expect(hook).toEqual({
      schemaVersion: 1, sessionId: 'codex-session', turnId: 'turn-1', type: 'tool.started',
      occurredAt: '2026-09-22T00:00:00.000Z', metadata: { toolName: 'Bash' },
    });
  ...
  it('does not infer completion from a tool failure field', () => {
    ...
    expect(normalized).toMatchObject({ type: 'tool.finished', metadata: { toolName: 'Bash' } });
