...
- Kimi Code CLI：`2.0.2`。`kimi web --help` 默认端口 58627，省略 `--host` 绑定 `127.0.0.1`，启用 bearer 鉴权并打印一次性 server token；实现不使用 `--dangerous-bypass-auth`。
...
- Kimi Code 登录、`kimi web` token 捕获、usage 查询与重启续期：**NOT_RUN**。尚未配置服务器授权卷。
